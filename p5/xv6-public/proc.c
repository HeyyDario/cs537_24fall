...
void exit(void)
{
  ...
  // Free all memory mappings and handle reference counts.
  // for (int i = 0; i < curproc->wmap_data.total_mmaps; i++)
  // {
  //   uint start = curproc->wmap_data.addr[i];
  //   uint end = start + curproc->wmap_data.length[i];

  //   for (uint va = start; va < end; va += PGSIZE)
  //   {
  //     pte_t *pte = get_pte(curproc->pgdir, (void *)va, 0);
  //     if (!pte || !(*pte & PTE_P))
  //       continue;

  //     uint pa = PTE_ADDR(*pte);
  //     decref(pa); // Decrement reference count and free if no references remain
  //     *pte = 0;   // Clear the PTE
  //   }
  // }

  // Free the process's page table and physical memory.
